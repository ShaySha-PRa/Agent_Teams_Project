...
import { uploadDocument, startParse, listPlaybooks } from '../api/documents';
...
export const UploadPage: React.FC = () => {
  ...
  const [validations, setValidations] = useState<{ label: string; status: string; detail: string }[]>([
    { label: '格式校验', status: 'pending' as string, detail: '等待上传' },
    { label: '文件大小', status: 'pending' as string, detail: '最大 50MB' },
    { label: '加密检测', status: 'pending' as string, detail: '自动检测' },
    { label: '损坏检测', status: 'pending' as string, detail: '自动检测' },
    { label: 'OCR 检测', status: 'pending' as string, detail: '自动检测' },
  ]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValidations(prev => prev.map((v, i) => {
      if (i === 0) return { ...v, status: isPdfDocx ? ('pass' as const) : ('fail' as const), detail: isPdfDocx ? `${ext?.toUpperCase()} 格式` : `不支持 ${ext}` };
      if (i === 1) return { ...v, status: sizeOk ? ('pass' as const) : ('fail' as const), detail: sizeOk ? `${(f.size/1024).toFixed(1)}KB` : '超过 50MB 限制' };
      return v;
    }));
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setValidations(prev => prev.map(v => ({ ...v, status: v.status === 'pending' ? ('pass' as const) : v.status })));
    } catch (e: any) {
  ...
  const handleParse = async () => {
    ...
    setError('');
    try {
      await startParse(docId, { playbook_id: 'pr_001', ocr_mode: 'immediate' }) as ApiResponse<any>;
        ...
        <div className="card-padded" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', background: 'var(--color-primary-light)', borderRadius: 8, border: '1px solid var(--color-primary)' }}>
              <span style={{ fontSize: 13 }}>配置摘要: NDA 协议 · {title || file?.name || '—'} · 手动触发</span>
