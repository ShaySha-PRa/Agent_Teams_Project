...
- 禁止：任务完成后还在重复输出相同内容（反复 echo "done"/"ok"/"."）
- 禁止：已经确认过状态后再次调用相同的检查命令
- 禁止：多轮无产出的确认性对话

**完成标准**：任务完成后，给出一次清晰总结后就停止，不要反复确认同一个结果。

### 7.2 状态汇报原则

- 汇报一次，不重复
- 已展示过的信息（git log / test results / server status）不要再次展示
- 用户 2 轮没有回应时，给简洁状态即可，不持续 ping

### 7.3 Git 操作规范

- 只有用户明确说"提交"/"commit"/"push"时才操作 git
- 不自动提交，不自动 push
- 用户明确要求暂停 git 操作时必须完全遵守

### 7.4 回复精简原则

- 无新信息就不回复
- 优先用 1-2 行总结，不用大段重复内容
- 能用表格/列表就不用长段落
